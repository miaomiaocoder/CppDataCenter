/*
 * 程序名：demo28.cpp，此程序用于演示基于HTTP协议的数据访问接口的简单实现。
 * 作者：任振华
 */
#include "_public.h"
#include "_ooci.h"
...
// 解析从GET请求中的参数，从T_ZHOBTMIND1表中查询数据，以xml格式返回客户端
...
int main(int argc, char *argv[])
{
    ...
    // 3、从T_ZHOBTMIND1表中查询数据，以xml格式返回客户端
    ...
    if (Writen(TcpServer.m_connfd, strsend, strlen(strsend)) == false)
        return -1;
...
bool SendData(const int sockfd, const char *strget)
{
    ...
    // http://127.0.0.1:8080/api?username=ren&passwd=405640081ren&intetname=getZHOBTMIND1&obtid=59981&begintime=20220414152532&endtime=20220414152532
    ...
    getvalue(strget, "intetname", intetname, 30); // 获取接口名
    ...
    // 连接数据库
    connection conn;
    conn.connecttodb("scott/tiger@snorcl11g_gz", "Simplified Chinese_China.AL32UTF8");

    // 准备查询数据的SQL。
    sqlstatement stmt(&conn);
    stmt.prepare("select '<obtid>'||obtid||'</obtid>'||'<ddatetime>'||to_char(ddatetime,'yyyy-mm-dd hh24:mi:ss')||'</ddatetime>'||'<t>'||t||'</t>'||'<p>'||p||'</p>'||'<u>'||u||'</u>'||'<keyid>'||keyid||'</keyid>'||'<endl/>' from T_ZHOBTMIND2 where obtid=:1 and ddatetime>to_date(:2,'yyyymmddhh24miss') and ddatetime<to_date(:3,'yyyymmddhh24miss')");
